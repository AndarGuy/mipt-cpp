...
#define OFFSET_FILE_HEADER 0x0
#define OFFSET_DIB_HEADER 0xE
...
typedef struct BITMAP_FILE_HEADER : I_OFFSETED {
    ...
    uint32_t get_offset() override { return OFFSET_FILE_HEADER; }
    ...
    /// @brief The method reads a FileHeader from provided stream and returns it.
    /// @param stream Provided stream to read.
    /// @return Unique pointer to FileHeader read from stream.
    static std::unique_ptr<FileHeader> read_header(std::istream&);

} FileHeader;

#define MAX_DIB_HEADER_SIZE 124
#define MIN_DIB_HEADER_SIZE 12

// Immediately follows the Bitmap file header
struct BITMAP_CORE_HEADER : DIBHeader {
...
struct BITMAP_INFO_HEADER2 : BITMAP_INFO_HEADER {
...
class DIBHeader : I_OFFSETED {
   private:
    std::unique_ptr<DIBHeader> parse_header();
   ...
   public:
    ...
    uint32_t get_offset() override { return OFFSET_DIB_HEADER; }

    static std::unique_ptr<DIBHeader> read_header(std::istream&);
