...
class ListGraph : public IGraph {
 private:
  std::vector<std::vector<size_t>> contiguity_list, reversed_contiguity_list;
  size_t vertices_amount;

 public:
  // Конструктор от числа вершин.
  ListGraph(size_t vertices_amount) noexcept;

  // Конструктор от указателя на базовый класс.
  ListGraph(const IGraph* graph) noexcept;

  // Конструктор копирования и копирующее присваивание должны быть явно запрещены.
  ListGraph(IGraph& graph) = delete;
