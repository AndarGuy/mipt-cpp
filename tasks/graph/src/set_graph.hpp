...
class SetGraph : public IGraph {
 private:
  std::vector<std::unordered_set<size_t>> contiguity_list, reversed_contiguity_list;
  size_t vertices_amount;

 public:
  // Конструктор от числа вершин.
  SetGraph(size_t vertices_amount) noexcept;

  // Конструктор от указателя на базовый класс.
  SetGraph(const IGraph* graph) noexcept;

  // Конструктор копирования и копирующее присваивание должны быть явно запрещены.
  SetGraph(IGraph& graph) = delete;
