...
class String {
 public:
  ...
  /// @brief Изменяет размер на `new_size`. Если вместимость не позволяет хранить столько символов, то выделяется новый
  /// буфер с вместимостью `new_size`. В случае `new_size` > `size` заполняет недостающие символы значением `filler`.
  /// @param new_size Новый размер строки.
  /// @param filler Символ заполнения.
  void resize(unsigned int new_size, char filler);

  /// @brief Изменяет `capacity` до `new_capacity`, если исходная вместимость меньше, и не изменяет объект иначе `size`
  /// не изменяется.
 ...
 private:
  ...
  /// @brief Реаллокация массива данных под новый размер.
